...
Rule-ADRs live in `docs/adr/` alongside classic decision-ADRs and share the same sequential numbering: `0001-slug.md`, `0002-slug.md`, and so on. Create `docs/adr/` lazily, only when the first ADR is written.
...
```md
# ADR-NNNN: {imperative rule title, for example "Handlers depend inward"}

**Rule:** {one imperative sentence, phrased as MUST or MUST NOT}. For example: "Code in `handlers/` MUST NOT import from `infra/`; depend on `core/` interfaces instead."

**Why:** {1-2 sentences naming the trade-off or principle this protects}.

**How to check:** {what a reviewer or a grep looks for to confirm compliance, concrete enough to verify against a diff}.
```

The filename slug is the rule's shorthand. Pick it so `docs/adr/` reads as an index of rules at a glance, using `0012-no-orm-in-domain.md` rather than `0012-database.md`.
...
- **Exceptions:** narrow, named carve-outs. A rule riddled with exceptions is a sign it's really two rules or none.
