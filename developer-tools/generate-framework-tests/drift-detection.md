...
Execute this check at the start of every invocation, immediately after scope is determined.

1. If the manifest file does not exist, proceed normally, since no tests have been generated yet.
...
3. Classify each difference into one of three buckets:
   - `+add`: a case name is in the proposed list but not in `cases[]`, meaning new behaviour appeared in the source that needs a test.
   - `-remove`: a case name is in `cases[]` but not in the proposed list, meaning the corresponding source behaviour was removed.
...
2. Include each orphan in the approval plan with a removal proposal, for example:
   > `tests/foo/test_bar.py`: source `src/foo/bar.py` was deleted, so propose removing the test file.
...
Write the manifest after all test files for this invocation have been successfully written. Never write it before.
