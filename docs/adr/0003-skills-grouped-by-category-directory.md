...
Skills live one level down, under a category directory — `kanban/` (the workflow chain), `developer-tools/` (everything else for coding), `behaviour/` (wording and interaction rules, borrowed by Skills in any other category), `schoolwork/` (study skills), and `archive/` (retired, never installed). `install.sh` walks a fixed `CATEGORIES` list and still symlinks each Skill to a **flat** `~/.claude/skills/<name>`, because Claude Code discovers Skills by bare directory name and does not read nested categories.
